...
| Opus 인코딩 | [audiopus](https://crates.io/crates/audiopus) v0.3 | 실시간 PCM → Opus 변환 (저지연) |
| HTTP 서버 | [tiny_http](https://crates.io/crates/tiny_http) v0.12 | 경량 HTTP 스트리밍 서버 |
| Windows GUI | [native-windows-gui](https://crates.io/crates/native-windows-gui) v1.0 | Windows 네이티브 설정 패널 |
...
2. **Opus 인코딩** (opus_encoder.rs)
   - audiopus 라이브러리로 실시간 Opus 인코딩
   - 10ms 프레임 (480 샘플) - 저지연 최적화
   - 기본 128kbps (설정 가능: 64~256kbps)
   - Raw Opus 패킷을 서버로 전송
...
3. **HTTP 스트리밍** (server.rs)
   ...
   - 클라이언트별 Ogg 컨테이너 생성 (고유 시리얼)
   - 다중 클라이언트 동시 지원

4. **설정 패널** (gui.rs)
   - Windows 네이티브 GUI (native-windows-gui)
   - 실시간 클라이언트 수 표시
   - 포트/비트레이트 설정
...
│   ├── opus_encoder.rs   # Opus 인코딩 + Ogg 컨테이너
│   ├── encoder.rs        # (레거시) MP3 인코딩
│   ├── server.rs         # HTTP 스트리밍 서버
│   ├── config.rs         # 설정 파일 관리
│   ├── gui.rs            # Windows 네이티브 설정 패널
│   └── tray.rs           # 시스템 트레이 아이콘
├── resources/
...
**주요 기능:**
- `new(sample_rate, channels, bitrate)` - Opus 인코더 초기화
- `encode_raw(samples)` - f32 PCM → Raw Opus 패킷 변환
- `get_headers_with_serial(serial)` - OpusHead + OpusTags Ogg 페이지 생성
- `wrap_opus_packet(packet, granule, serial, seq)` - Opus 패킷을 Ogg 페이지로 래핑
- `create_ogg_page(...)` - 수동 Ogg 페이지 생성 (CRC-32 포함)

**Opus 설정:**
- Application 모드: `LowDelay` (실시간 스트리밍 최적화)
- 프레임 크기: 10ms (최저 지연)
- 지원 비트레이트: 64, 96, 128, 192, 256 kbps

**Ogg 컨테이너:**
- 클라이언트별 고유 시리얼 번호
- BOS(Beginning of Stream) 플래그는 첫 페이지에만
- CRC-32 체크섬 (polynomial 0x04C11DB7)
...
**Ogg 스트리밍 특징:**
- 클라이언트 연결 시 새 Ogg 스트림 시작 (고유 시리얼)
- OpusHead + OpusTags 헤더 먼저 전송
- Raw Opus 패킷을 Ogg 페이지로 래핑하여 전송
...
시스템 트레이 아이콘 및 폴백 처리:
- 아이콘 파일 로딩 (`resources/rustcast_envelope.ico`)
- 임베디드 리소스 폴백 (릴리즈 빌드)
- 콘솔 모드 폴백 (아이콘 없을 시)
...
| 레이턴시 | Opus 인코딩 ~40ms (블루투스 수준) | ✅ 최적화 완료 |
| 콘솔 창 (디버그) | 개발 빌드에서 콘솔 표시 | 릴리즈에서 해결 |
| CMake 필요 | Opus 빌드에 CMake 필수 | 문서화됨 |
...
- **Ogg 컨테이너 호환성**: Chromium은 BOS 플래그가 첫 페이지에만 있어야 함 - 수동 Ogg 페이지 생성으로 해결
- **저지연 최적화**: 채널 버퍼 크기 축소 (64→4), Opus LowDelay 모드, 10ms 프레임
- **클라이언트별 스트림**: 각 클라이언트가 고유한 Ogg 시리얼로 완전한 스트림 수신
